...
// Basic AST Nodes
type ASTNode = FunctionCallNode | StringLiteralNode | IdentifierNode
...
class Parser {
  ...
  private isSpecialIdentifierChar(char: string): boolean {
    ...
    // This is to maintain backward compatibility with loose args
  ...
  public parse(): ASTNode[] {
    ...
    while (this.pos < this.input.length) {
      ...
      if (node) {
        ...
        // If we can't parse an expression, try to consume the invalid chunk to avoid infinite loop
        // Often happens if there are trailing characters
        this.consume()
      }
      this.skipWhitespace()
    }
  ...
  private parseExpression(): ASTNode | null {
    ...
    // Attempt to parse function call or bare unquoted string identifier
    ...
    if (this.peek() === '(') {
      ...
      if (!this.match(')')) {
        do {
          this.skipWhitespace()
          // For loose arguments, we might just have unquoted text
          const argNode = this.parseExpression()
          if (argNode) {
            args.push(argNode)
          } else {
            // If we couldn't parse structured node, try to gather a loose string until comma or paren
            const loose = this.parseLooseString()
            if (loose) {
              args.push({ type: 'StringLiteral', value: loose } as StringLiteralNode)
            }
          }
          this.skipWhitespace()
        } while (this.match(','))
        ...
        if (!this.match(')')) {
          throw new Error(`Expected ')' after arguments for function ${id}`)
        }
      }
      return {
        type: 'FunctionCall',
    ...
    // It was just an identifier / loose string
    return { type: 'StringLiteral', value: id } as StringLiteralNode
  }

  private parseIdentifier(): string {
    let result = ''
    while (this.pos < this.input.length && this.isSpecialIdentifierChar(this.peek())) {
      result += this.consume()
    }
    return result.trim()
  }

  private parseLooseString(): string {
    let result = ''
    while (this.pos < this.input.length) {
      const char = this.peek()
      if (char === ',' || char === ')') break
  ...
  private parseStringLiteral(): StringLiteralNode | null {
    const quoteStart = this.consume() // consume quote
    let value = ''
...
export function executeFormula(formula: string, selection: string = ''): FormulaResult {
  ...
  try {
    ...
    // Find the first top-level function call
    const rootCall = ast.find(n => n.type === 'FunctionCall') as FunctionCallNode
    if (!rootCall) {
      return { success: false, error: 'Invalid formula structure' }
    }

    return { success: true, value: evaluateAST(rootCall, selection) }
...
/**
 * Evaluates an AST node to a string
 */
function evaluateAST(node: ASTNode, selection: string): string {
  if (node.type === 'StringLiteral') {
    if (node.value.toLowerCase() === 'selection') {
      return selection
    }
    return node.value
  }
  ...
  if (node.type === 'Identifier') {
    if (node.name.toLowerCase() === 'selection') {
      return selection
    }
    return node.name
  }
  ...
  if (node.type === 'FunctionCall') {
    ...
    if (node.args.length === 0) {
      // Implicit 'selection' argument if empty parens
      processedArgs = [selection]
    } else {
      processedArgs = node.args.map(argNode => evaluateAST(argNode, selection))
    }
...
export function validateFormula(formula: string): FormulaValidation {
  if (!formula || !formula.trim()) {
    return { valid: false, error: 'Formula cannot be empty', functions: [] }
  }
  ...
  try {
    ...
    if (ast.length === 0) {
      return { valid: false, error: 'No valid function found in formula', functions: [] }
    }
    ...
    const visit = (node: ASTNode) => {
      if (node.type === 'FunctionCall') {
        if (!VALID_FUNCTION_NAMES.includes(node.name)) {
          isValid = false
          errorMsg = `Unknown function: ${node.name}. Valid functions: ${VALID_FUNCTION_NAMES.join(', ')}`
        } else {
    ...
    if (!isValid) {
      return { valid: false, error: errorMsg, functions }
    }

    return { valid: true, functions }
  } catch (err) {
    return { valid: false, error: `Parse error: ${String(err)}`, functions: [] }
  }
}
