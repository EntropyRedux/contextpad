...
/**
 * Annotation to mark transactions that should bypass the locked block filter.
 * Used for internal operations like unlocking all blocks or toggling locks.
 */
export const isInternalFilterBypass = Annotation.define<boolean>()
    ...
    enter: (node) => {
      if (node.name === 'FencedCode') {
        ...
        if (parsed.params.lock === true) {
          // LOCK THE ENTIRE NODE (including fences)
          const contentStart = node.from
          ...
          if (contentEnd > contentStart) {
            ...
            exclusions.forEach(ex => {
              ...
              if (handler) {
                // For exclusions (like variables), we still only want to allow editing INSIDE the content,
                // so we use the original content offsets for the handler
                const headerLine = doc.lineAt(node.from)
                const innerStart = Math.min(node.to, headerLine.to + 1)
                let innerEnd = node.to
                const endLine = doc.lineAt(node.to)
                if (/^(`{3,}|~{3,})/.test(endLine.text)) {
                  innerEnd = Math.max(innerStart, endLine.from - 1)
                }
                
                handler(doc, innerStart, innerEnd, allowedBuilder)
              }
            })
          }
        }
