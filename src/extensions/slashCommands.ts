...
export interface SlashCompletion extends Completion {
      ...
      apply: async (view: EditorView, _completion: Completion, from: number, to: number) => {
        try {
          if (isButton) {
            ...
            try {
              const result = await executeAction(action.code, view, action.id)
...
export function slashCommandCompletions(context: CompletionContext): CompletionResult | null {
  ...
  const allOptions: SlashCompletion[] = getSlashCompletions()
    ...
    options: allOptions.map(opt => ({
      ...
      render: (completion: Completion, state: any) => {
        ...
        if (opt.type === 'function') {
          const customType = opt.typeLabel
          type = customType || 'CMD'
        }
