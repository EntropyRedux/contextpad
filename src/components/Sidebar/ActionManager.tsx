...
`
  ...
  const handleInsertButton = (id: string) => {
    const activeTab = getActiveTab()
    if (activeTab?.editorView) {
      const view = activeTab.editorView
      const pos = view.state.selection.main.head
      const code = `[[action:${id}]]`
      view.dispatch({
        changes: { from: pos, insert: code },
        selection: { anchor: pos + code.length }
      })
      view.focus()
      addNotification({ type: 'success', message: 'Button code inserted' })
    } else {
      addNotification({ type: 'warning', message: 'No active editor' })
    }
  }
  ...
  const handleExecute = async (actionId: string) => {
    ...
    try {
      const result = await executeAction(action.code, activeTab.editorView, action.id)
      if (result.success) {
        addNotification({ type: 'success', message: 'Action Executed', details: action.name })
      } else {
            ...
            actions={
              <>
                <button
                  ...
                  onClick={() => action.type === 'button' ? handleInsertButton(action.id) : handleExecute(action.id)}
                  disabled={!action.enabled}
                  aria-label={action.type === 'button' ? `Insert ${action.name}` : `Execute ${action.name}`}
                  title={action.type === 'button' ? 'Insert into editor' : 'Execute'}
                >
                  {action.type === 'button' ? <Plus size={12} /> : <Play size={12} fill="currentColor" />}
                </button>
                <button
