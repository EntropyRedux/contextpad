...
import {
  ...
  Calculator, Star, Download, Globe
} from 'lucide-react'
...
export function MenuBar() {
  ...
  const handleExportBlueprint = async () => {
    ...
    const blueprint = generateBlueprint(activeTab.content, activeTab.title)
    const json = JSON.stringify(blueprint, null, 2)
    const defaultName = `${activeTab.title.replace(/\.[^/.]+$/, "")}.blueprint.json`
  ...
  const handleExportHTML = async () => {
    ...
    try {
      ...
      const tocHtml = viewSettings.previewShowTOC ? generateTOC(activeTab.content) : ''

      const resolvedTheme = viewSettings.previewTheme === 'match'
      ...
      if (filePath) {
        ...
        addNotification({ type: 'success', message: 'HTML exported successfully' })
      }
    } catch (error) {
      console.error('Export HTML error:', error)
      addNotification({ type: 'error', message: 'Failed to export HTML' })
    }
    setActiveMenu(null)
  }
  ...
  const handleRunAction = async (actionCode: string) => {
    if (activeTab?.editorView) {
      try {
        const result = await executeAction(actionCode, activeTab.editorView)
        if (result.success) {
          addNotification({ type: 'success', message: 'Action Executed' })
        } else {
          addNotification({ type: 'error', message: 'Action Failed', details: result.error })
        }
      } catch (err) {
        addNotification({ type: 'error', message: 'Action Error', details: String(err) })
      }
    }
    setActiveMenu(null)
  }
      ...
      <div className={styles.menuItem}>
        ...
        {activeMenu === 'file' && (
          <div className={styles.dropdown}>
            ...
            <div className={styles.separator} />
            <button onClick={() => handleMenuItemClick(toggleLivePreview)} style={{ color: viewSettings.showLivePreview ? '#4ec9b0' : 'inherit' }}>
              <Globe size={14} className={styles.menuIcon} style={{ color: viewSettings.showLivePreview ? '#4ec9b0' : 'inherit' }} />
              {viewSettings.showLivePreview ? 'Stop Live Preview' : 'Start Live Preview'}
            </button>
