...
export function useFileDrop() {
  ...
  const openFileInTab = useCallback(async (filePath: string) => {
    // Prevent opening if already being processed
    if (processingFilesRef.current.has(filePath)) {
      return
    }
    ...
    try {
      ...
      // Small delay before removing from processing to handle near-simultaneous events
      setTimeout(() => {
