...
export function useFileOperations() {
  const { addTab, updateTab, getActiveTab, addRecentFile, setOpenFolderPath } = useTabStore(
    useShallow(state => ({
      addTab: state.addTab,
      updateTab: state.updateTab,
      getActiveTab: state.getActiveTab,
      addRecentFile: state.addRecentFile,
      setOpenFolderPath: state.setOpenFolderPath
    }))
  )
  ...
  const openFile = async () => {
    try {
      ...
      // Extract folder path from file path securely
      const folderPath = await invoke<string>('get_dir_name', { path: filePath })
