...
const normalizeActionCode = (code: string): string => {
  ...
  return code
    // Legacy helper typo support
    .replace(/\bhelpers\.toCopy\s*\(/g, 'helpers.copyToClipboard(')
    .replace(/\bhelpers\.copy\s*\(/g, 'helpers.copyToClipboard(')
    // Legacy direct clipboard API support
    .replace(/\bnavigator\.clipboard\.writeText\s*\(/g, 'helpers.copyToClipboard(')
}

const normalizeAction = <T extends Pick<Action, 'code'>>(action: T): T => ({
  ...action,
  code: normalizeActionCode(action.code)
})

const sanitizeImportedAction = (
  importAction: any,
  index: number
): Omit<Action, 'id' | 'createdAt' | 'updatedAt' | 'isDefault'> | null => {
  if (!importAction || typeof importAction !== 'object') {
    return null
  }

  const code = typeof importAction.code === 'string' ? normalizeActionCode(importAction.code) : ''
  if (!code.trim()) {
    return null
  }

  const type: ActionType = importAction.type === 'button' ? 'button' : 'command'
  const name = typeof importAction.name === 'string' && importAction.name.trim().length > 0
    ? importAction.name.trim()
    : `Imported Action ${index + 1}`

  const description = typeof importAction.description === 'string' ? importAction.description : ''
  const categoryRaw = typeof importAction.category === 'string' && importAction.category.trim().length > 0
    ? importAction.category
    : 'GENERAL'
  ...
  return {
    ...
    enabled: importAction.enabled !== false
  }
}
...
const loadPersistedActions = (): Action[] => {
  try {
    ...
    if (saved) {
      return (JSON.parse(saved) as Action[]).map(normalizeAction)
    }
  } catch (error) {
  ...
  return {
      ...
      set((state) => {
        const actions = state.actions.map(a =>
          a.id === id ? { 
            ...a, 
            ...updates, 
            code: updates.code !== undefined ? normalizeActionCode(updates.code) : a.code,
            category: updates.category ? updates.category.toUpperCase() : a.category,
            updatedAt: Date.now() 
          } : a
        )
        persistActions(actions)
    ...
    importActions: (jsonString, behavior = 'skip') => {
      try {
        ...
        const importedActions = Array.isArray(data)
          ? data
        ...
        importedActions.forEach((importAction: any, index: number) => {
          const normalizedImportAction = sanitizeImportedAction(importAction, index)
          if (!normalizedImportAction) {
            invalidCount++
            return
          }

          const existingIndex = actionsToSave.findIndex(a => 
            a.name.trim().toLowerCase() === normalizedImportAction.name.trim().toLowerCase()
          )

          if (existingIndex !== -1) {
            duplicates.push(normalizedImportAction.name)
            if (behavior === 'overwrite') {
              // Update existing
              actionsToSave[existingIndex] = {
                ...actionsToSave[existingIndex],
                ...normalizedImportAction,
                category: (normalizedImportAction.category || actionsToSave[existingIndex].category).toUpperCase(),
            ...
            added.push({
              ...normalizedImportAction,
              category: (normalizedImportAction.category || 'GENERAL').toUpperCase(),
              id: crypto.randomUUID(),
              isDefault: false,
              createdAt: Date.now(),
              updatedAt: Date.now()
        ...
        if (invalidCount > 0) {
          console.warn(`[Action Import] Skipped ${invalidCount} invalid action(s) with missing/invalid code.`)
        }

        return { 
          added: added.length, 
          updated: updatedCount, 
          skipped: invalidCount + (behavior === 'skip' ? duplicates.length : 0), 
          duplicates 
        }
